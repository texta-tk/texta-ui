...
export class ApplyToIndexDialogComponent implements OnInit, OnDestroy {
  ...
  applyForm = new UntypedFormGroup({
    ...
    addSpansFormControl: new UntypedFormControl(),
  ...
  ngOnInit(): void {
    this.projectStore.getCurrentProject().pipe(filter(x => !!x), take(1), switchMap(proj => {
      if (proj) {
        ...
        return this.regexTaggerService.applyToIndexOptions(proj.id, this.data.id);
      }
      return of(null);
    })).subscribe(options => {
  ...
  onSubmit(formData: SubmitFormModel): void {
    ...
    this.regexTaggerService.applyToIndex(this.currentProject.id, this.data.id, body).subscribe(resp => {
