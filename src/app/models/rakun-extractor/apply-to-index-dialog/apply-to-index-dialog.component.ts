...
export class ApplyToIndexDialogComponent implements OnInit, OnDestroy {
  ...
  applyForm = new UntypedFormGroup({
    ...
    addSpansFormControl: new UntypedFormControl(false),
  ...
  constructor(private dialogRef: MatDialogRef<ApplyToIndexDialogComponent>,
              ...
              @Inject(MAT_DIALOG_DATA) public data: RakunExtractor,
  ...
  ngOnInit(): void {
    this.projectStore.getCurrentProject().pipe(filter(x => !!x), take(1), switchMap(proj => {
      if (proj) {
        ...
        return this.rakunExtractorService.applyToIndexOptions(proj.id, this.data.id);
      }
      return of(null);
    })).subscribe(options => {
  ...
  onSubmit(formData: SubmitFormModel): void {
    const body = {
      ...
      add_spans: formData.addSpansFormControl,
    ...
    this.rakunExtractorService.applyToIndex(this.currentProject.id, this.data.id, body).subscribe(resp => {
